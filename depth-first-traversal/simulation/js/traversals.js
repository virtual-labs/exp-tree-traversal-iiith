...
// X-order: Root-Right-Left traversal (fixed for children array)
export function xOrderTraversal(node, result = []) {
  if (!node) return;
  result.push(node.name); // Visit root
  console.log("Visiting node (X-order):", node.name);
  if (node.children && node.children[1])
    xOrderTraversal(node.children[1], result); // Visit right
  if (node.children && node.children[0])
    xOrderTraversal(node.children[0], result); // Visit left
  return result;
}
