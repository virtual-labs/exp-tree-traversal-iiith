import { xOrderTraversal } from "./traversals.js";
...
function generateGraph() {
  ...
  console.log("Random tree index:", random);
  root = treeData[random];
  update(treeData[random]);
  setCorrectSequence(root); // Set correct sequence for the current tree
  console.log("Correct X-order sequence set to:", tree_traversal.sequence_list);
  if (document.getElementById("preorder")) preorder();
  if (document.getElementById("bft")) bft();
}

function setCorrectSequence(root) {
  tree_traversal.sequence_list = [];
  xOrderTraversal(root, tree_traversal.sequence_list);
  console.log(
    "setCorrectSequence called. Sequence:",
    tree_traversal.sequence_list
  );
}
...
function submit() {
  // Log the user's sequence and the correct sequence for debugging
  const userSequence = document
    .getElementById("traversal")
    .innerText.trim()
    .split(/\s+/);
  console.log("User sequence (from traversal):", userSequence);
  ...
  if (index == tree_traversal.sequence_list.length - 1) {
    if (tree_traversal.error == 0) {
      ...
      console.log("Validation: FAIL (error flag set)");
    }
  } else {
    document.getElementById("commentbox").style.display = "block";
    document.getElementById("comments").innerHTML =
      "Traversal Incomplete.Your Traversal is incorrect.Try again! \n Re-attempt practice section.";
    console.log("Validation: FAIL (incomplete traversal)");
  }
}
...
function check(d) {
  ...
  index += 1;
  console.log(
    `User clicked node: ${d.name}, Expected: ${tree_traversal.sequence_list[index]}, Index: ${index}`
  );
  if (index == tree_traversal.sequence_list.length - 1) {
    visitElement(d, 0);
    ...
    visitElement(d, 0);
    console.log("Mismatch detected! Marking error flag.");
  } else {
    visitElement(d, 0);
  }
}
...
function visitElement(element, animX) {
  if (!tree_traversal.sequence_list[index]) return;
...
function recursiveInorder(root) {
  if (root !== undefined) {
    if (root.children !== undefined) recursiveInorder(root.children[0]);
    tree_traversal.sequence_list.push(root.name);
    console.log("called", tree_traversal.sequence_list);
    if (root.children !== undefined) recursiveInorder(root.children[1]);
  }
}
function recursivePreorder(root) {
  ...
  if (root !== undefined) {
    tree_traversal.sequence_list.push(root.name);
    if (root.children !== undefined) recursivePreorder(root.children[1]);
    if (root.children !== undefined) recursivePreorder(root.children[0]);
  }
}
function recursivePostorder(root) {
  console.log("called", root);
  if (root !== undefined) {
    if (root.children !== undefined) recursivePostorder(root.children[0]);
    if (root.children !== undefined) recursivePostorder(root.children[1]);
    tree_traversal.sequence_list.push(root.name);
  }
}
